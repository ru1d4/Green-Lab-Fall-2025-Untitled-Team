...
summary_stats <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    min = min(cold_start_energy, na.rm = TRUE),
    max = max(cold_start_energy, na.rm = TRUE),
    mean = mean(cold_start_energy, na.rm = TRUE),
    median = median(cold_start_energy, na.rm = TRUE),
    sd = sd(cold_start_energy, na.rm = TRUE),
    .groups = "drop"
  )
...
shapiro_results <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    p_value = tryCatch(shapiro.test(cold_start_energy)$p.value, error = function(e) NA)
  ) %>%
  ungroup() %>%
  mutate(
...
kw_result <- dat_dependent_var %>%
  group_by(benchmark) %>%
  summarize(
    kw = list(kruskal.test(cold_start_energy ~ compilation, data = cur_data())),
...
bench_series <- dat_dependent_var %>%
  group_by(benchmark, compilation) %>%
  summarize(
    values = list(cold_start_energy),
    median_energy = median(cold_start_energy),
    .groups = "drop"
  )
