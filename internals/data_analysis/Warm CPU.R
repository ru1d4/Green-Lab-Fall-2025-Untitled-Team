...
summary_stats <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    min = min(warm_start_cpu_util, na.rm = TRUE),
    max = max(warm_start_cpu_util, na.rm = TRUE),
    mean = mean(warm_start_cpu_util, na.rm = TRUE),
    median = median(warm_start_cpu_util, na.rm = TRUE),
    sd = sd(warm_start_cpu_util, na.rm = TRUE),
    .groups = "drop"
  )
...
shapiro_results <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    p_value = tryCatch(shapiro.test(warm_start_cpu_util)$p.value, error = function(e) NA)
  ) %>%
  ungroup() %>%
  mutate(
...
kw_result <- dat_dependent_var %>%
  group_by(benchmark) %>%
  summarize(
    kw = list(kruskal.test(warm_start_cpu_util ~ compilation, data = cur_data())),
...
kw_result_dunn <- dat_dependent_var %>%
  ...
  summarize(
    dunn = list(dunnTest(
      warm_start_cpu_util ~ compilation,
      data = cur_data(),
      method = "bonferroni"
    )),
    .groups = "drop"
...
bench_series <- dat_dependent_var %>%
  group_by(benchmark, compilation) %>%
  summarize(
    values = list(warm_start_cpu_util),
    median_energy = median(warm_start_cpu_util),
    .groups = "drop"
  )
