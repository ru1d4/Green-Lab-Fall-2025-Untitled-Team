...
summary_stats <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    min = min(warm_start_energy, na.rm = TRUE),
    max = max(warm_start_energy, na.rm = TRUE),
    mean = mean(warm_start_energy, na.rm = TRUE),
    median = median(warm_start_energy, na.rm = TRUE),
    sd = sd(warm_start_energy, na.rm = TRUE),
    .groups = "drop"
  )
...
shapiro_results <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    p_value = tryCatch(shapiro.test(warm_start_energy)$p.value, error = function(e) NA)
  ) %>%
  ungroup() %>%
  mutate(
...
kw_result <- dat_dependent_var %>%
  group_by(benchmark) %>%
  summarize(
    kw = list(kruskal.test(warm_start_energy ~ compilation, data = cur_data())),
...
bench_series <- dat_dependent_var %>%
  group_by(benchmark, compilation) %>%
  summarize(
    values = list(warm_start_energy),
    median_energy = median(warm_start_energy),
    .groups = "drop"
  )
