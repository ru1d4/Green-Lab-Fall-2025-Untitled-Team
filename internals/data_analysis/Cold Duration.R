...
dat_dependent_var <- dat_data %>%
  select(benchmark, compilation, cold_start_duration) %>%
  mutate(
    benchmark = factor(benchmark),
    compilation = factor(compilation)
  )
...
summary_stats <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    min = min(cold_start_duration, na.rm = TRUE),
    max = max(cold_start_duration, na.rm = TRUE),
    mean = mean(cold_start_duration, na.rm = TRUE),
    median = median(cold_start_duration, na.rm = TRUE),
    sd = sd(cold_start_duration, na.rm = TRUE),
    .groups = "drop"
  )
...
# Volin + Box
ggplot(dat_dependent_var, aes(x = compilation, y = cold_start_duration, fill = compilation)) +
...
shapiro_results <- dat_dependent_var %>%
  ...
  summarise(
    n = n(),
    p_value = tryCatch(shapiro.test(cold_start_duration)$p.value, error = function(e) NA)
  ) %>%
  ungroup() %>%
  mutate(
...
kw_result <- dat_dependent_var %>%
  group_by(benchmark) %>%
  summarize(
    kw = list(kruskal.test(cold_start_duration ~ compilation, data = cur_data())),
...
kw_result_dunn <- dat_dependent_var %>%
  ...
  summarize(
    dunn = list(dunnTest(
      cold_start_duration ~ compilation,
      data = cur_data(),
      method = "bonferroni"
    )),
    .groups = "drop"
...
bench_series <- dat_dependent_var %>%
  group_by(benchmark, compilation) %>%
  summarize(
    values = list(cold_start_duration),
    median_energy = median(cold_start_duration),
    .groups = "drop"
  )
